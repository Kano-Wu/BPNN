...
public class BpNN2_iris2 {
	...
	public static void loadData() {
		...
		try {
			...
			System.out.println("训练、测试数据集导入ok!");
			br.close();
		} catch (Exception ex) {
	...
	public static void loadTrainData() {
		trainNum = 80;
		File file = new File("datasets\\Iris_data_train.txt");
		FileReader fr = null;
		try {
			fr = new FileReader(file.getAbsolutePath());
		} catch (FileNotFoundException e) {
	...
	public static void loadTestData() {
		testNum = 20;
		File file = new File("datasets\\Iris_data_test.txt");
		FileReader fr = null;
		try {
			fr = new FileReader(file.getAbsolutePath());
		} catch (FileNotFoundException e) {
	 ...
	 * 打乱数据集的顺序，函数有误！！
	 */
	public static void shuffleData(double arr[][], int col){
		 for(int i=0;i<arr[0].length;i++){
             int ran1=(int)(Math.random()*arr.length);
             int ran2=(int)(Math.random()*arr.length);
             for(int j=0;j<col;j++){
            	 double temp=arr[ran1][j];
            	 arr[ran1][j]=arr[ran2][j];
            	 arr[ran2][j]=temp;
             }
		 }
	}
